...
if (not dead) {
	...
	if(!place_meeting(x, y+1, obj_wall) && !place_meeting(x, y+8, obj_fakeWall)) {
		vsp += grav;
	} else {
		if (key_up) {
			vsp = jumpheight;
	...
	if(place_meeting(x+hsp, y, obj_wall) || place_meeting(x+hsp, y, obj_fakeWall)) {
		while(!place_meeting(x+sign(hsp), y, obj_wall) && !place_meeting(x+sign(hsp), y, obj_fakeWall)) {
			x += sign(hsp);
		}
		hsp = 0;
	}
	...
	//==== Vertical Collision
	if(place_meeting(x, y+vsp, obj_wall) || place_meeting(x, y+vsp, obj_fakeWall)) {
		while(!place_meeting(x, y+sign(vsp), obj_wall) && !place_meeting(x, y+sign(vsp), obj_fakeWall)) {
			y += sign(vsp);
		}
		vsp = 0;
	}
